...
var specificArticleSpec = function specificArticleSpec() {
  ...
  it('test response if there is no article matching the given id', function (done) {
    _chai["default"].request(_server["default"]).get('/api/v1/articles/invalidId').set('x-access-token', token).end(function (err, res) {
      var body = res.body;
...
var _default = specificArticleSpec;
